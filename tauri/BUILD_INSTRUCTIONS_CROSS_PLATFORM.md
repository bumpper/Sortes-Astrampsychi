# Cross-Platform Build Instructions for Sortes Astrampsychi

This guide provides detailed instructions for building the Sortes Astrampsychi application on Windows, macOS, and Linux (including Fedora for RPM packages).
...
The Sortes Astrampsychi application is built using Tauri 2.0, which allows you to create native desktop applications for multiple platforms. However, **you must build on each target platform** - cross-compilation from Windows to macOS/Linux is not straightforward with Tauri.
...
- **MSI Installer:** `src-tauri\target\release\bundle\msi\sortes-astrampsychi_1.0.0_x64_en-US.msi`
- **NSIS Installer:** `src-tauri\target\release\bundle\nsis\sortes-astrampsychi_1.0.0_x64-setup.exe`
- **Executable:** `src-tauri\target\release\sortes-astrampsychi.exe`
...
- **DMG Installer:** `src-tauri/target/release/bundle/dmg/sortes-astrampsychi_1.0.0_aarch64.dmg`
- **App Bundle:** `src-tauri/target/release/bundle/macos/sortes-astrampsychi.app`
...
codesign --force --deep --sign "Developer ID Application: Your Name" \
  src-tauri/target/release/bundle/macos/sortes-astrampsychi.app

# Verify signature
codesign --verify --verbose src-tauri/target/release/bundle/macos/sortes-astrampsychi.app
```
...
- **DEB Package:** `src-tauri/target/release/bundle/deb/sortes-astrampsychi_1.0.0_amd64.deb`
- **AppImage:** `src-tauri/target/release/bundle/appimage/sortes-astrampsychi_1.0.0_amd64.AppImage`

### Installing the DEB Package

```bash
sudo dpkg -i src-tauri/target/release/bundle/deb/sortes-astrampsychi_1.0.0_amd64.deb
```
...
- **RPM Package:** `src-tauri/target/release/bundle/rpm/sortes-astrampsychi-1.0.0-1.x86_64.rpm`
- **AppImage:** `src-tauri/target/release/bundle/appimage/sortes-astrampsychi_1.0.0_amd64.AppImage`

### Installing the RPM Package

```bash
sudo dnf install src-tauri/target/release/bundle/rpm/sortes-astrampsychi-1.0.0-1.x86_64.rpm
```

Or using rpm directly:

```bash
sudo rpm -i src-tauri/target/release/bundle/rpm/sortes-astrampsychi-1.0.0-1.x86_64.rpm
```
